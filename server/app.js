...
function startApp() {
    ...
    app.post('/api/trip', async function (req, res) {
        ...
        if (!validator.isIn(tripInfo.who_are_you, ['Worker', 'Employer']))
            validationError.push('Wrong who are you')
        if (!validator.isLength(tripInfo.email, { min: 0, max: 20 }))
